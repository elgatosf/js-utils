...
export async function listWindowsProcesses(): Promise<ProcessInfo[]> {
	const script = "Get-CimInstance Win32_Process | Select-Object ProcessId,CommandLine,Name | ConvertTo-Json -Compress";
	...
	try {
		...
		const parsed = JSON.parse(stdout) as Array<WindowsProcessInfo> | WindowsProcessInfo;
